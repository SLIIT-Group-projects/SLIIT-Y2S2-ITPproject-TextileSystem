...
const LorryManagement = () => {
    ...
    const fetchLorries = async () => {
        try {
            const response = await axios.get('http://localhost:3000/lorries');
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.post('http://localhost:3000/lorries/add', formData);
            setFormData({
                lorryNumber: '',
                capacity: 0,
                driverName: ''
    ...
    const handleDelete = async (id) => {
        try {
            await axios.delete(`http://localhost:3000/lorries/delete/${id}`);
            fetchLorries();
        } catch (error) {
